...
const handleChecking = async (id: string) => {
  ...
  try {
    await axios.post('/franchise/check', { id });
...
<template>
  <v-row>
    <v-col cols="12" md="12">
      <UiParentCard title="Danh sách đăng ký nhượng quyền">
        <div class="pa-7 pt-0">
          <v-row>
            <v-col cols="12" sm="3">
              <v-select
                ...
                <v-table class="month-table table-post">
                  <thead>
                    <tr>
                      ...
                      <th class="text-subtitle-1 font-weight-bold">Tỉnh thành</th>
                  ...
                  <tbody>
                    <tr v-for="(item, index) in list" :key="item._id" class="month-item">
                      ...
                      <td>
                        <h6 class="text-body-1 text-muted">{{ item.province }}</h6>
                      </td>
                      <td>
