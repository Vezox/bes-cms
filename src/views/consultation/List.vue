...
const objects: any = {
  lt18: 'Học sinh THPT',
  gt18: 'Sinh viên',
  lt26: 'Người đi làm(<26t)',
  gt26: 'Người đi làm(>26t)',
  other: 'Chủ doanh nghiệp/nội trợ/làm tự do'
};

const targets: any = {
  target_1: 'Tiếng Anh tổng quát & Luyện thi',
  target_2: 'Sự nghiệp & mở rộng quan hệ',
  target_3: 'Tiếng anh văn phòng',
  target_4: 'Giải trí & giao lưu',
  target_5: 'Du học',
  target_6: 'Du lịch & văn hoá',
  target_7: 'Phong cách sống & Giải trí'
};
                ...
                <v-table class="month-table table-post">
                  <thead>
                    <tr>
                      ...
                      <th class="text-subtitle-1 font-weight-bold">Đối tượng</th>
                      <th class="text-subtitle-1 font-weight-bold">Mục tiêu</th>
                  ...
                  <tbody>
                    <tr v-for="(item, index) in list" :key="item._id" class="month-item">
                      ...
                      <td>
                        <h6 class="text-body-1 text-muted">{{ item.address }}</h6>
                      </td>
                      <td>
                        <h6 class="text-body-1 text-muted">{{ objects[item.object] }}</h6>
                      </td>
                      <td>
                        <h6 class="text-body-1 text-muted">{{ item.targets.map((i: any) => targets[i]).join(', ') }}</h6>
                      </td>
                      <td>
                        <h6 class="text-body-1 text-muted">{{ new Date(item.created_time).toLocaleString() }}</h6>
                      </td>
                      <td>
                        <div v-if="item.checked" style="color: blue">Đã tư vấn</div>
                        <v-btn v-else style="color: green" :loading="item.loading" @click="handleChecking(item._id)"> Tư vấn </v-btn>
                      </td>
                    </tr>
                  </tbody>
                </v-table>
